...
class WAM(object):
    ...
    def __init__(self):
        ...
        self.joint_angle_bound = np.array([[-2.2, 2.2], [-1.8, 1.8], [-2.5, 2.5], [-0.7, 1.2], [-0.5, 0.5], [-0.5, 0.5], [-0.5, 0.5]]) # the reference angle is the zero pose
        # the joint bounds from barrett are: [[-2.6, 2.6], [-2, 2], [-2.8, 2.8], [-0.9, 3.1], [-4.76, 1.24], [-1.6, 1.6], [-3.0, 3.0]]
        self.num_joints = 7
...
class DataRecorder(object):
    ...
    def joint_traj(self, t, f=0.1, A=[0.5]):
        f *= 2 * math.pi
        q = np.sin(f*t) * np.array([1, 1, 1, 1, 0, 0, 0]) * A
        qdot = f * np.cos(f*t) * np.array([1, 1, 1, 1, 0, 0, 0]) * A
        qdotdot = -f**2 * np.sin(f*t) * np.array([1, 1, 1, 1, 0, 0, 0]) * A
        return q, qdot
    ...
    def joint_traj1(self, t, T=10):
        ...
        factor = 0.7
        q = np.array([theta1, theta2, theta3*0.7, theta4, 0, 0, 0]) * factor
        qdot = np.array([thetad1, thetad2, thetad3*0.7, thetad4, 0, 0, 0]) * factor
        qdotdot = np.array([thetadd1, thetadd2, thetadd3*0.7, thetadd4, 0, 0, 0]) * factor
