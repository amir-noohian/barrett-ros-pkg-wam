...
'''
This code is for the WAM leader with the first joint puck broken.
'''
...
class WAM(object):
    ...
    def __init__(self):
        ...
        self.joint_angle_bound = np.array([[-1.8, 1.8], [-2.0, 2.0], [-0.7, 0.7]]) # the reference angle is the zero pose.
        self.num_joints = 3
    ...
    def joint_pos_cmd(self, pos_goal: np.ndarray):
        ...
        msg = RTJointPos()
        msg.joints = pos_goal
        msg.rate_limits = np.array([500.0]*3)
    ...
    def go_zero(self):
        self._wait_for_joint_states()
        self.joint_move([0.0, 0.0, 0.0])

    def stop(self):
        self.joint_vel_cmd([0.0, 0.0, 0.0])
...
class DataRecorder(object):
    ...
    def joint_traj(self, t, f=0.1, A=0.6):
        f *= 2 * math.pi
        q = A * np.array([np.sin(f*t), 0, np.sin(f*t)])
        qdot = A * f * np.array([np.cos(f*t), 0, np.cos(f*t)])
        return q, qdot
    ...
    def joint_traj1(self, t, T=10):
        ...
        factor = 1.0
        q = np.array([theta2, 0, theta4]) * factor
        qdot = np.array([thetad2, 0, thetad4]) * factor
        # qdotdot = np.array([thetadd2, 0, thetadd4]) * factor
    ...
    def collect_dynamics(self):
        ...
        initial_angle, initial_velocity = self.joint_traj1(0)
